...
public class GeneDescriptionFuserLongestString extends AttributeValueFuser<String, Gene, Attribute> {

    public GeneDescriptionFuserLongestString() {
    ...
    public void fuse(RecordGroup<Gene, Attribute> group, Gene fusedRecord, Processable<Correspondence<Attribute, Matchable>> schemaCorrespondences, Attribute schemaElement) {
        ...
        fusedRecord.setGeneDescription(fused.getValue());

        // add provenance info
        fusedRecord.setAttributeProvenance(Gene.GENEDESCRIPTION, fused.getOriginalIds());
    }

    @Override
    public boolean hasValue(Gene record, Correspondence<Attribute, Matchable> correspondence) {
        return record.hasValue(Gene.GENEDESCRIPTION);
    }

    @Override
    public String getValue(Gene record, Correspondence<Attribute, Matchable> correspondence) {
        return record.getGeneDescription();
    }
