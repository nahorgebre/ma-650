...
public class Disease extends AbstractRecord<Attribute> implements Serializable {
    ...
    public String toString() {
        return String.format("[Disease: %s / %s / %s / %s / %s / %s / %s / %s / %s / %s / %s / %s / %s /]",
            getDiseaseIdUMLS(), getDiseaseName(), getDiseaseSpecificityIndex(),
            getDiseasePleiotropyIndex(), getDiseaseTypeDisGeNET(), getDiseaseClassMeSH(),
            getDiseaseSemanticTypeUMLS(), getAssociationScore(), getEvidenceIndex(),
            getYearInitialReport(), getYearFinalReport(), getPmId(), getSource());
    }
