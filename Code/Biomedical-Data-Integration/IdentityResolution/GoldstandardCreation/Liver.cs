        ...
        public static void mart_export_liver_2_gene2pubtatorcentral() {
            for (int i = 1; i <= Publication.pubTatorPartitionSize; i++)
            {
                ...
                if (!File.Exists(trueFile) | !File.Exists(falseFile))
                {
                    ...
                    (List<Goldstandard> trueList, List<Goldstandard> falseList) = Methods.compareFilesPubTator(Datasets.mart_export_liver_path, Datasets.getGene2pubtatorcentral_path(i), 3);
                    Methods.createOuput(trueFile, trueList);
                    Methods.createOuput(falseFile, falseList);
                }       
            }
