...
namespace PrepareGoldStandard
{
    public class AWSupload
    {
        ...
        public static void run()
        {
            ...
            DirectoryInfo goldstandardDirectory = new DirectoryInfo(string.Format("{0}/data/output", Environment.CurrentDirectory));

            foreach (FileInfo file in goldstandardDirectory.GetFiles())
            {
                
                string keyName = string.Format("named-entity-recognition/evaluation/{0}", file.Name);
