...
namespace Data_Preparation
{
    public class AWSupload
    {
        ...
        {
            DirectoryInfo goldstandardDirectory = new DirectoryInfo(string.Format("{0}/data/output/goldstandard", Environment.CurrentDirectory));
            foreach (FileInfo file in goldstandardDirectory.GetFiles())
            {
                string keyName = string.Format("named-entity-recognition/input/goldstandard/{0}", file.Name);      
                string bucketName = "nahorgebre-ma-650-master-thesis";
                UploadFileAsync(bucketName, file.FullName, keyName).Wait();
            }

            DirectoryInfo textDirectory = new DirectoryInfo(string.Format("{0}/data/output/text", Environment.CurrentDirectory));
            foreach (FileInfo file in textDirectory.GetFiles())
            {
                string keyName = string.Format("named-entity-recognition/input/text/{0}", file.Name);      
                string bucketName = "nahorgebre-ma-650-master-thesis";
                UploadFileAsync(bucketName, file.FullName, keyName).Wait();
            }
        }
