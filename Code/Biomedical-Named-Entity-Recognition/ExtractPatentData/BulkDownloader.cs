                ...
                using (var reader = new StreamReader(string.Format("{0}/data/source/{1}.txt", Environment.CurrentDirectory, year.ToString())))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        if (!File.Exists(filePath))
                        {
                            ...
                            Console.WriteLine("{0}Downloading: {1} - {2}", Environment.NewLine, filePath, DateTime.UtcNow.ToString());
                        }
                        else
                        {
                            if (!isZipFileValid(filePath))
                            {
                                ...
                                Console.WriteLine("{0}Downloading: {1}", Environment.NewLine, filePath);
                            }
                        }
                    }
                }               
